...
let
  ...
  workspaceSwitchBinds = lib.concatMap (
    i:
    let
  ...
  workspaceMoveBinds = lib.concatMap (
    i:
    let
      key = if i == 10 then "0" else toString i;
    in
    [ "\$mainMod ALT, ${key}, movetoworkspace, ${toString i}" ]
  ) wsRange;
  ...
  parseBindList =
    type: lines: map (line: (keybindLib.parseBindString line) // { _type = type; }) lines;
  ...
  parseShellBinds =
    ...
    map (
      line:
      (keybindLib.parseBindString line)
      // {
        _type = type;
        _shell = "caelestia";
      }
