...
Notable controls:
- `settings.userSettings.<name>.dev.git.*`: per-user git identity and host-based include rules
- `settings.userSettings.<name>.dev.git.hostProfiles.<name>.*`: host, per-host `userName`/`userEmail`, and SSH key mapping
