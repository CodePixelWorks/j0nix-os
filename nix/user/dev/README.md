...
- `settings.userSettings.<name>.dev.ssh.agent.provider = "auto"`: use GNOME keyring in graphical sessions and OpenSSH in pure console sessions
...
- `SSH_ASKPASS`/`SUDO_ASKPASS` point at the GNOME askpass binary when the SSH agent provider is `gnome-keyring` or `auto`
...
- Baseline user dev packages include terminal multiplexers (`tmux`, `screen`) and Go/Rust toolchains (`go`, `cargo`, `rustc`)
...
- The automatic loader waits for every selected agent socket and retries `ssh-add`; `auto` loads keys into both agents so graphical and console sessions remain independent
...
- ComfyUI MCP (`comfy` server in `~/.hermes/config.yaml`): the server binary comes from the curated hermes-package (`pkgs.comfy-mcp`, bundles the comfy-cli engine); without the private hermes input it falls back to a `uvx`-based wrapper. Env wiring: `COMFYUI_URL` (server-side submit/job tools) and `COMFY_LOCAL_URL` (engine-side verbs) both point at the configured ComfyUI host/port, `COMFY_MCP_REMOTE_SHARED_MODELS=1` marks the models dir as shared storage
