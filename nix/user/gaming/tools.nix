...
EOF
  ...
  # ===========================================================================
  # Time hack (libfaketime LD_PRELOAD)
  # ===========================================================================
  timeHack = gaming.timeHack or { };
  timeHackEnabled = timeHack.enable or false;
  timeHackDefaultOffset = timeHack.defaultOffset or "+4h";

  fakeTimeWrapper = pkgs.writeShellScriptBin "game-time-fake" ''
    set -eu

    usage() {
      cat >&2 <<EOF
usage: game-time-fake [options] <command> [args...]

Wraps libfaketime via LD_PRELOAD. Only the launched process tree sees
the faked time; the system clock is untouched. Works for native Linux
binaries and Proton/Wine games.

Offset format (libfaketime): +N[s|m|h|d|y] or -N[s|m|h|d|y]
  Examples: +4h, -30m, +1d, +2h30m

Options:
  --offset <expr>   Offset from now (default: ${timeHackDefaultOffset})
  --static          Freeze time at launch instead of advancing offset
  --show            Print the fake time the target will see, then exec
  --help            Show this help

Environment overrides:
  GAME_TIME_OFFSET  Same as --offset
  GAME_TIME_STATIC  When set (any value), same as --static

Steam launch options examples:
  game-time-fake --offset +4h %command%
  game-time-fake --offset -30m %command%
EOF
      exit 2
    }
    ...
    while [ $# -gt 0 ]; do
      case "$1" in
        ...
        --)
          ...
          ;;
        -*)
          echo "game-time-fake: unknown option: $1" >&2
          usage
          ;;
        *)
          break
    ...
    case "$offset_expr" in
      @*|+*|-*)
        ;;
      *)
        echo "game-time-fake: offset must start with @, +, or - (got: $offset_expr)" >&2
        exit 2
        ;;
    esac
    ...
    if [ "$show" = "1" ]; then
      fake_now="$(date)"
      echo "game-time-fake: FAKE=$fake_now (offset=$offset_expr, static=$static_mode)" >&2
    fi

    exec "$@"
  '';

  fakeTimeAtWrapper = pkgs.writeShellScriptBin "game-time-fake-at" ''
    set -eu

    usage() {
      cat >&2 <<EOF
usage: game-time-fake-at <iso-time> <command> [args...]

Launches <command> with a faked absolute clock time.
<iso-time> is any libfaketime absolute timestamp, e.g.:
  2026-01-15 14:30:00
  @2026-01-15T14:30:00
...
EOF
    ...
    echo "game-time-fake-at: FAKE=@$target_time" >&2
    exec "$@"
  '';

  fakeTimeNow = pkgs.writeShellScriptBin "game-time-now" ''
    set -eu
