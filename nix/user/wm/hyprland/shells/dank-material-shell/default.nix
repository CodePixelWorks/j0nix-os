...
let
  ...
  wallpaperPath =
    dmsWallpaper.wallpaperPath
      or "/run/current-system/sw/share/wallpapers/nix-wallpaper-stripes-logo.png";
...
in
{
    ...
    listMerge.mergeUnique [
      ...
      (
        (
          ...
          ++ lib.optional ((pkgs ? qt5) && (pkgs.qt5 ? qtwayland)) pkgs.qt5.qtwayland
        )
        ++ (
          with pkgs;
          lib.optionals separateMode [
  ...
  systemd.user.services.dank-material-shell =
    lib.mkIf (integratedMode && dmsUseSystemd && hasPackage)
      {
        Unit = {
        ...
        Service = {
          ExecStart = "${dmsBinaryFromPackage} run";
          Restart = "on-failure";
          RestartSec = "2s";
          Environment = [
  ...
  home.file.".config/quickshell/dms".source = config.lib.file.mkOutOfStoreSymlink dmsConfigSource;
  ...
  home.activation.dmsInfo = lib.hm.dag.entryAfter [ "writeBoundary" ] ''
    $DRY_RUN_CMD echo "${
      if integratedMode then
        "DMS integrated mode enabled. Use dms-start/dms-stop."
      else
        "DMS separate mode enabled. Use dms-install once, then dms-start/dms-stop/dms-uninstall."
    }"
  '';
  ...
  home.activation.dmsSystemdReconcile = lib.hm.dag.entryAfter [ "reloadSystemd" "writeBoundary" ] (
    lib.optionalString (integratedMode && dmsUseSystemd) ''
      if [ -x "${pkgs.systemd}/bin/systemctl" ]; then
        ...
        if [ -n "$resolved_unit" ]; then
          ...
          $DRY_RUN_CMD ${pkgs.systemd}/bin/systemctl --user restart "$resolved_unit" || $DRY_RUN_CMD ${pkgs.systemd}/bin/systemctl --user start "$resolved_unit" || true
        fi
      fi
    ''
  );
  ...
  assertions = [
    {
      assertion = builtins.elem dmsMode [
        "integrated"
        "separate"
      ];
    ...
    {
      assertion = builtins.elem dmsStartupMode [
        "systemd"
        "exec-once"
      ];
    ...
    {
      assertion = builtins.elem wallpaperFillMode [
        "PreserveAspectCrop"
        "PreserveAspectFit"
        "Stretch"
      ];
    ...
    {
      assertion = (!integratedMode) || hasPackage;
      message = ''
        Integrated DMS mode requires inputs.dank-material-shell.packages.${pkgs.stdenv.hostPlatform.system}.default.
      '';
    }
  ];
}
