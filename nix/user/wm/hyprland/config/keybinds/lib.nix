...
let
  ...
  parseBindString =
    ...
    let
      ...
      argument_ = if n > 3 then lib.concatStringsSep ", " (lib.drop 3 parts) else null;
    in
    {
      mods = mods_;
      key = key_;
    ...
    let
      ...
      catOf =
        ...
        let
          ...
          key = b.key or "";
        in
        if lib.hasPrefix "workspace" d then
          "Navigation"
        else if lib.hasPrefix "movefocus" d || lib.hasPrefix "movewindow" d then
          "Window Management"
        else if lib.hasPrefix "resizeactive" d || lib.hasPrefix "splitratio" d then
          "Resizing"
        else if lib.hasPrefix "layoutmsg" d then
          "Layout"
        else if lib.hasPrefix "fullscreen" d || lib.hasPrefix "togglefloating" d then
          "Window State"
        else if d == "killactive" then
          "Window State"
        else if d == "exit" then
          "Session"
        else if d == "exec" && (lib.hasInfix "screenshot" arg || key == "Print") then
          "Screenshot"
        else if
          d == "exec"
          && (lib.hasInfix "volume" arg || lib.hasInfix "mute" arg || lib.hasPrefix "XF86Audio" key)
        then
          "Media"
        else if d == "exec" && (lib.hasInfix "brightness" arg || lib.hasPrefix "XF86MonBrightness" key) then
          "Media"
        else if d == "exec" && (lib.hasInfix "clipboard" arg || lib.hasInfix "emoji" arg) then
          "Launcher"
        else if d == "exec" && (lib.hasInfix "lock" arg || lib.hasInfix "caelestia:session" arg) then
          "Session"
        else if d == "global" then
          "Shell"
        else if d == "exec" then
          "App Launch"
        else
