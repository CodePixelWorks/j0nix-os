...
let
  ...
  # Pinned Sunshine release, newer than the nixpkgs snapshot this flake is
  # built against. Based on the nixpkgs by-name expression. buildDepsTag must
  # match the third-party/build-deps submodule commit pinned by the release.
...
stdenv'.mkDerivation (finalAttrs: {
  ...
  ui = buildNpmPackage {
    ...
    pname = "sunshine-ui";
    # provisional hash: computed against package.json/package-lock.json of the
    # release; nix reports the real hash on first build if it differs.
  ...
  cmakeFlags = [
    ...
    # system tray needs Qt; Sunshine runs headless here, so drop it
  ...
  passthru.tests = { inherit (nixosTests) sunshine; };
