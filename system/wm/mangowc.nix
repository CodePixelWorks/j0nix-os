...
in
{
  ...
  environment.etc."greetd/mango.conf" =
    lib.mkIf (manageOwnDisplayManager && useGreetd && selectedGreetdGreeter == "dms-greeter")
      {
        text = ''
  ...
  assertions = [
    ...
    {
      assertion =
        (!(manageOwnDisplayManager && useGreetd))
        || builtins.elem selectedGreetdGreeter dm.validGreetdGreeters;
