...
- On this NVIDIA setup, Wayland capture modes (`wlr`, `wl`, `wayland`, and the headless `auto` path) intentionally avoid the privileged `cap_sys_admin` wrapper, because that wrapper breaks userspace codec loading such as `libnvidia-encode.so.1`.
...
- In wlroots/Wayland capture mode the module intentionally skips the privileged `cap_sys_admin` wrapper, because that capture path does not need it and the wrapper interferes with NVIDIA/CUDA userspace library resolution.
