...
- On this NVIDIA setup, the local `sunshine` package overlay adds `/run/opengl-driver` to the binary RUNPATH so NVENC libraries such as `libnvidia-encode.so.1` stay visible even when Sunshine uses the privileged `cap_sys_admin` path for KMS capture.
