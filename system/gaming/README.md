...
- For the physical-output dummy-plug path, this host currently opts into `capture = "kms"` to prefer the direct DRM capture path on the dedicated stream output. If that regresses to a blank stream again, `wlr` remains the fallback capture mode for Wayland.
