...
- In the Hyprland headless path, `capture = "auto"` is still treated as a Wayland/non-privileged launch for the service wrapper. That keeps Sunshine off the `cap_sys_admin` wrapper, which otherwise breaks NVIDIA userspace codec loading on this setup.
